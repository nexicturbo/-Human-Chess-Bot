...
class LichessGrabber(Grabber):
    ...
    def get_current_game_id(self):
        """Extract game ID from Lichess URL to detect game changes."""
        try:
            url = self.chrome.current_url
            if "/training/" in url:
                game_id = url.split("/training/")[-1].split("?")[0].split("/")[0]
                return f"puzzle_{game_id}"

            url_path = url.split("lichess.org/")[-1].split("?")[0].split("#")[0]
            game_id = url_path.split("/")[0]
        ...
        if is_puzzles:
            ...
            if not move_list_elem:
                if self.moves_list:
                    self.reset_moves_list()
                return []
            if self.tag_name is None and self.set_moves_tag_name() is False:
                return []

        # Get the move elements (children of the move list element)
        try:
            if not is_puzzles:
                all_children = move_list_elem.find_elements(By.CSS_SELECTOR, self.tag_name)
            else:
                all_children = move_list_elem.find_elements(By.CSS_SELECTOR, "move")
        except NoSuchElementException:
            return None

        if self.moves_list and len(all_children) < len(self.moves_list):
            print(f"[DEBUG] Move list shrank ({len(self.moves_list)} -> {len(all_children)}), resetting")
            self.reset_moves_list()
            if not is_puzzles and (self.tag_name is None and self.set_moves_tag_name() is False):
                return []
            try:
                if not is_puzzles:
                    all_children = move_list_elem.find_elements(By.CSS_SELECTOR, self.tag_name)
